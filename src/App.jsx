...
function optimizeRoute(stops) {
  ...
  while (unvisited.length > 0) {
    ...
    unvisited.forEach((s,i) => {
      if (!s.lat||!s.lng||!last.lat||!last.lng) return;
      const d = Math.sqrt((s.lat-last.lat)**2+(s.lng-last.lng)**2);
...
function pointInCircle(lat,lng,cLat,cLng,rDeg) {
  return Math.sqrt((lat-cLat)**2+(lng-cLng)**2) <= rDeg;
}
          ...
          <button onClick={() => {
              ...
              setShowLasso(prev => {
                ...
                map.once("mousedown", e => {
                  ...
                  const onMove = e2 => {
                    const d = startLL.distanceTo(e2.latlng);
                    if (circle) map.removeLayer(circle);
                    circle = L.circle(startLL, { radius:d, color:"#cc2222", weight:2, fillColor:"#cc2222", fillOpacity:0.1, dashArray:"6 4" }).addTo(map);
                  };
                  const onUp = e2 => {
                    map.off("mousemove", onMove);
                    map.getContainer().style.cursor = "";
                    map.dragging.enable();
                    const rDeg = startLL.distanceTo(e2.latlng) / 111320;
                    const inside = companies.filter(c => c.lat && c.lng && pointInCircle(c.lat, c.lng, startLL.lat, startLL.lng, rDeg));
                    if (inside.length === 0) { alert("No companies in that area — try a larger circle."); if (circle) map.removeLayer(circle); setShowLasso(false); return; }
                    setStops(optimizeRoute(inside));
                    setTimeout(() => { if (circle) map.removeLayer(circle); }, 1500);
                    setShowLasso(false);
                  };
            ...
            style={{padding:"5px 14px",background:showLasso?"#1a0a0a":"none",border:`1px solid ${showLasso?"#cc2222":"#2a2a2a"}`,borderRadius:5,color:showLasso?"#cc2222":"#555",cursor:"pointer",fontSize:10,fontFamily:"monospace",fontWeight:700,letterSpacing:1}}>
            {showLasso ? "✕ CANCEL" : "⭕ LASSO"}
          </button>
          {stops.length > 0 && (
